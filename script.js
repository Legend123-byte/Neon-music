...
let currentLyrics = []; // To store {time, text}
...
function setupEventListeners() {
    ...
    if (lyricsBtn) {
        lyricsBtn.addEventListener('click', () => {
            if (currentView === 'lyrics') {
                ...
                if (song) fetchLyrics(song);
            }
        });
    }

    const lyricsLanguageSelect = document.getElementById('lyrics-language-select');
    const mobileLyricsLangSelect = document.getElementById('mobile-lyrics-language-select');

    if (lyricsLanguageSelect) {
        lyricsLanguageSelect.addEventListener('change', (e) => {
            const lang = e.target.value;
            if (mobileLyricsLangSelect) mobileLyricsLangSelect.value = lang;
            if (!originalLyricsData.length && !originalPlainLyrics) return;
            translateAndRenderLyrics(lang);
        });
    }

    if (mobileLyricsLangSelect) {
        mobileLyricsLangSelect.addEventListener('change', (e) => {
            const lang = e.target.value;
            if (lyricsLanguageSelect) lyricsLanguageSelect.value = lang;
            if (!originalLyricsData.length && !originalPlainLyrics) return;
            translateAndRenderLyrics(lang);
        });
    }
    ...
    if (lyricsBackBtn) {
        lyricsBackBtn.addEventListener('click', goBack);
    }
}
...
function updateMobilePlayerUI(song) {
    ...
    // Update background gradient based on average color of the album art.
    const overlay = document.getElementById('mobile-player-overlay');

    // Reset background to simple dark if cover is missing
    if (!song.cover || !overlay) return;

    // Use a hidden canvas to extract pixel color
    const img = new Image();
    ...
    img.onload = () => {
        try {
            ...
            // Darken it slightly to mimic the spotify gradient vibe
            r = Math.max(0, Math.floor(r * 0.7));
            g = Math.max(0, Math.floor(g * 0.7));
            b = Math.max(0, Math.floor(b * 0.7));

            // Apply Spotify-like gradient
            overlay.style.background = `linear-gradient(180deg, rgb(${r}, ${g}, ${b}) 0%, #121212 100%)`;
        } catch (err) {
            console.error('Cannot extract color from image:', err);
            overlay.style.background = 'linear-gradient(180deg, #1e1e2e, #000)';
        }
    };
    img.src = song.cover;
}
...
function updateMobileProgress() {
    ...
    if (audio.duration) {
        ...
        if (durTime) durTime.innerText = '-' + formatTime(audio.duration - audio.currentTime);

        // Sync lyrics for mobile if in lyrics mode
        const overlay = document.getElementById('mobile-player-overlay');
        if (overlay && overlay.classList.contains('lyrics-mode')) {
...
function updateProgress() {
    ...
    if (mobileDur) mobileDur.innerText = '-' + formatTime(duration - currentTime);
...
async function fetchLyrics(song) {
    ...
    const titleEl = document.getElementById('lyrics-view-title');
    const artistEl = document.getElementById('lyrics-view-artist');

    if (titleEl) titleEl.innerText = song.title || 'Unknown Title';
    if (artistEl) artistEl.innerText = song.artist || 'Unknown Artist';

    container.innerHTML = '<div style="margin-top: 50px;"><i class="fa-solid fa-spinner fa-spin" style="font-size: 40px; color: var(--primary-color);"></i><p style="margin-top:20px; font-size: 16px;">Finding lyrics...</p></div>';
    currentLyrics = [];
    originalLyricsData = [];
    originalPlainLyrics = "";
    ...
    const langSelect = document.getElementById('lyrics-language-select');
    const mLangSelect = document.getElementById('mobile-lyrics-language-select');
    if (langSelect) langSelect.value = 'original';
    if (mLangSelect) mLangSelect.value = 'original';
    ...
    try {
        ...
        let mapped = null;
        if (typeof lyricsMapping !== 'undefined') {
            mapped = lyricsMapping.find(m => m.id === song.id);
        }

        // If the user provided hardcoded custom lyrics mapping, use them directly and skip the API
        if (mapped && (mapped.customLrc || mapped.customPlain)) {
            if (mapped.customLrc) {
                originalLyricsData = parseLRC(mapped.customLrc);
                currentLyrics = [...originalLyricsData];
                renderLyrics(currentLyrics);
            } else if (mapped.customPlain) {
                originalPlainLyrics = mapped.customPlain;
                container.innerHTML = `<p style="white-space: pre-wrap; font-size: 18px; line-height: 2;">${originalPlainLyrics}</p>`;
            }
        ...
        if (mapped) {
            trackName = mapped.lrclibTitle || trackName;
            artistName = mapped.lrclibArtist || artistName;
        }
        ...
        if (!res.ok) {
            ...
            // If no data, try with just the track name
            if (!searchData || searchData.length === 0) {
                searchUrl = `https://lrclib.net/api/search?q=${encodeURIComponent(trackName)}`;
                searchRes = await fetch(searchUrl);
                searchData = searchRes.ok ? await searchRes.json() : [];

                // If STILL no data, try to take only before parentheses/hyphens
                if (!searchData || searchData.length === 0) {
                    const cleanTrackName = trackName.split('-')[0].split('(')[0].trim();
                    searchUrl = `https://lrclib.net/api/search?q=${encodeURIComponent(cleanTrackName)}`;
                    searchRes = await fetch(searchUrl);
                    searchData = searchRes.ok ? await searchRes.json() : [];
                }
            }
        ...
        if (data.syncedLyrics) {
            originalLyricsData = parseLRC(data.syncedLyrics);
            currentLyrics = [...originalLyricsData];
            renderLyrics(currentLyrics);
        } else if (data.plainLyrics) {
            originalPlainLyrics = data.plainLyrics;
            container.innerHTML = `<p style="white-space: pre-wrap; font-size: 18px; line-height: 2;">${originalPlainLyrics}</p>`;
        } else {
        ...
        const errHtml = '<div style="margin-top: 50px;"><i class="fa-solid fa-microphone-slash" style="font-size: 40px; color: #555;"></i><p style="margin-top:20px; font-size: 16px;">Lyrics not found</p></div>';
        if (container) container.innerHTML = errHtml;
        const mobileContainer = document.getElementById('mobile-lyrics-sheet-container');
...
async function translateTextChunks(textArray, targetLang) {
    if (targetLang === 'original' || !textArray.length) return textArray;
    ...
    for (const text of textArray) {
        if (currentLength + text.length > 1500) {
    ...
    // Determine API params for Hinglish
    const isHinglish = targetLang === 'hinglish';
    const tl = isHinglish ? 'hi' : targetLang; // For hinglish we translate to Hindi first
    const dtRm = isHinglish ? '&dt=rm' : '';

    for (const chunk of chunks) {
        const textToTranslate = chunk.join('\n');
        try {
            const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=${tl}&dt=t${dtRm}&q=${encodeURIComponent(textToTranslate)}`;
            const res = await fetch(url);
            const data = await res.json();

            let translatedText = '';

            if (isHinglish && data && data[0]) {
                // Find the romanized string which is usually in an array starting with 2 nulls
                let rmArr = data[0].find(item => item && item[0] === null && item[1] === null && typeof item[2] === 'string');
                if (rmArr) {
                    translatedText = rmArr[2];
                } else if (data[0].length > 1 && data[0][1] && typeof data[0][1][2] === 'string') {
                    translatedText = data[0][1][2]; // Fallback location
                } else {
                    // Fallback to translated text
                    data[0].forEach(item => {
                        if (item[0] && typeof item[0] === 'string') translatedText += item[0];
                    });
                }
            } else if (data && data[0]) {
                data[0].forEach(item => {
            ...
            // If API returns fewer or more lines due to joining errors, we pad or truncate to match chunk length to keep sync intact
            while (translatedLines.length < chunk.length) translatedLines.push(chunk[translatedLines.length]);
            translatedArray.push(...translatedLines.slice(0, chunk.length));
        } catch (e) {
            console.error("Translation API error", e);
            translatedArray.push(...chunk);
        }
    }
...
async function translateAndRenderLyrics(targetLang) {
    const container = document.getElementById('lyrics-container');
    const mobileContainer = document.getElementById('mobile-lyrics-sheet-container');

    if (originalLyricsData.length > 0) {
        ...
        const msg = '<div style="margin-top: 50px;"><i class="fa-solid fa-spinner fa-spin" style="font-size: 40px; color: var(--primary-color);"></i><p style="margin-top:20px; font-size: 16px;">Translating...</p></div>';
        if (container) container.innerHTML = msg;
        if (mobileContainer) mobileContainer.innerHTML = msg;

        const textsToTranslate = originalLyricsData.map(l => l.text);
        const translatedTexts = await translateTextChunks(textsToTranslate, targetLang);

        currentLyrics = originalLyricsData.map((l, i) => ({
            time: l.time,
            text: translatedTexts[i] || l.text
        }));

        renderLyrics(currentLyrics);
    } else if (originalPlainLyrics) {
        if (targetLang === 'original') {
            const txt = `<p style="white-space: pre-wrap; font-size: 18px; line-height: 2;">${originalPlainLyrics}</p>`;
            if (container) container.innerHTML = txt;
            if (mobileContainer) mobileContainer.innerHTML = txt;
            return;
        }
        ...
        const lines = originalPlainLyrics.split('\n');
        const translatedLines = await translateTextChunks(lines, targetLang);
        const txt = `<p style="white-space: pre-wrap; font-size: 18px; line-height: 2;">${translatedLines.join('\n')}</p>`;
        if (container) container.innerHTML = txt;
        if (mobileContainer) mobileContainer.innerHTML = txt;
    }
}
...
function renderLyrics(lyrics) {
    const container = document.getElementById('lyrics-container');
    const mobileContainer = document.getElementById('mobile-lyrics-area-content');
    ...
    lyrics.forEach((line, index) => {
        ...
        if (mobileContainer) {
            ...
            mp.className = 'mobile-lyric-line';
            mp.dataset.time = line.time;
            mp.dataset.index = index;
            mp.innerText = line.text;
            mp.addEventListener('click', () => {
                audio.currentTime = line.time;
                if (audio.paused) togglePlay();
            });
            mobileContainer.appendChild(mp);
        }
    });
}
...
function syncLyrics(currentTime) {
    ...
    const mobileLines = document.querySelectorAll('.mobile-lyric-line');
    ...
    if (activeIndex !== -1) {
        ...
        if (mobileLines.length > 0) {
            mobileLines.forEach((p, idx) => {
                if (idx === activeIndex) {
                    if (!p.classList.contains('active')) {
                        p.classList.add('active');
                        // auto scroll to center
                        const container = document.getElementById('mobile-lyrics-area-content');
                        if (container) {
                            const offset = p.offsetTop - container.offsetTop - (container.clientHeight / 2) + (p.clientHeight / 2);
...
const Visualizer = {
    ...
    `;
                        ...
                        // This `previewAudio` variable is not defined, it should be `audio`
...
function setupMobileFullPlayer() {
    ...
    function closeMobilePlayer() {
        ...
        overlay.classList.remove('lyrics-mode'); // Ensure lyrics mode is off when closing player
    }
    ...
    // Lyrics link click and Full Mode Logic
    const mobileLyricsLink = document.getElementById('mobile-lyrics-link');
    const lyricsArea = document.getElementById('mobile-lyrics-area');
    const artwork = document.querySelector('.mobile-player-artwork');


    if (mobileLyricsLink) {
        mobileLyricsLink.addEventListener('click', (e) => {
            e.stopPropagation();

            // 1. Tapping Animation
            mobileLyricsLink.classList.add('tapping');
            setTimeout(() => {
                mobileLyricsLink.classList.remove('tapping');
            }, 150);

            // 2. Toggle Lyrics Mode
            const isLyricsMode = overlay.classList.contains('lyrics-mode');

            if (!isLyricsMode) {
                overlay.classList.add('lyrics-mode');

                // 3. Fetch lyrics natively if not already present
                let song = null;
                if (typeof currentPlaylist !== 'undefined' && currentPlaylist[currentSongIndex]) {
                    song = currentPlaylist[currentSongIndex];
                } else if (audio.dataset.currentSongId && typeof songs !== 'undefined') {
                    song = songs.find(s => String(s.id) === String(audio.dataset.currentSongId));
                }

                if (song && (!currentLyrics.length && !originalPlainLyrics)) {
                    fetchLyrics(song);
                ...
                overlay.classList.remove('lyrics-mode');
            }
        });

        // Toggle back when tapping on the mini artwork or info area in lyrics mode
    ...
    // Swipe Down to Close Overlay (prevent closing player if lyrics mode is on?)
    const swipeTargets = [
